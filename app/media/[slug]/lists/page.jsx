...
const SingleLists = dynamic(() => import('@/components/media/singleMedia/singleLists'), { ssr: false })
...
export async function generateMetadata({ params}) {
  ...
  return {
    title:`لیست‌ها - ${data.media.title} `,
     description:`${data.media.summary}`,
     alternates: {
        canonical: `${siteURL}/media/${params.slug}/lists`,
     },
     robots: {
        index: true,
        follow: true,
  ...
  return (
    <div className="min-h-screen">
     {(!data) ? <Loading style={'h-screen'} /> : 
        <div>
        <SingleHeader media={data.media} season={data.seasons} page={'lists'}  >
              <SingleLists media_id={data.media._id} />
        </SingleHeader>
        </div>      
